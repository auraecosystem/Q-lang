...
class QRuntime:
    """Small tree-walking VM for Q-lang programs."""
    def __init__(self):
        self.parser = QParser()
        self.engine = QEngine()
        self.env = {}
    ...
    def _eval(self, node):
        ...
        if kind == "if":
            body = node[2] if self._eval(node[1]) else node[3]
        ...
        if kind == "while":
            ...
            while self._eval(node[1]):
                ...
                if count > self.max_loop_iterations:
                    raise RuntimeError("while loop exceeded safety iteration limit")
                result = self._eval_block(node[2])
            return result
        if kind == "define":
            obj = self.engine.registry.register(node[2], node[1], body=node[3])
            return obj
    ...
    def _semantic(self, operation, value):
        ...
        if operation == "coordinate": return {"coordinated": True, "action": value}
    ...
    def _call(self, name, args):
        if name == "print":
            print(*args); return args[-1] if args else None
        ...
        old = self.env.copy(); self.env.update(zip(params, args))
        result = None
        try:
            for statement in body: result = self._eval(statement)
...
if __name__ == "__main__":
    ...
    except (QSyntaxError, RuntimeError, QReturn) as exc:
